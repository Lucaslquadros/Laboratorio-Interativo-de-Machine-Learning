...
X = df.drop(columns=['Email','Address','Yearly Amount Spent'])
...
sns.pairplot(df, x_vars=df.drop(columns=['Email','Address','Yearly Amount Spent']), y_vars='Yearly Amount Spent',aspect=0.7)
plt.show()
...
- Avg. Session Length → linear positiva moderada

- Time on App → linear positiva moderada

- Time on Website → relação fraca

- Length of Membership → relação linear forte

✔ De modo geral, a suposição de linearidade pode ser considerada atendida
'''
...
plt.ylim(-30,30)
plt.xlim(254,726)
sns.lineplot(x=[254,726],y=[0,0],color='blue')
plt.show()
...
✔ Suposição atendida: análise do gráfico e teste estatístico. 
...
'''
Pelo gráfico, podemos observar uma curva normal quase perfeita.
...
plt.ylim(-30,30)
plt.xlim(254,726)
sns.lineplot(x=[254,726],y=[0,0],color='red')
...
sns.heatmap(df.drop(columns=['Email','Address']).corr(), annot=True,cmap='RdYlGn',square=True)
plt.show()

'''
Esses dados não contêm multicolinearidade perfeita entre as variáveis independentes. 
Caso houvesse, tentaríamos remover uma das variáveis correlacionadas, dependendo de 
qual fosse mais importante para o nosso modelo de regressão.
...
'''
CONCLUSÃO: Portanto, os principais pressupostos da Regressão Linear foram cumpridos com sucesso. 
Nesse caso, podemos afirmar que, apesar de haver poucos dados, eles são bem representativos. Além disso, 
as variáveis independentes conseguem explicar a dependente.
'''

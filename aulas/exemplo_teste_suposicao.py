...
'''
O conjunto de dados contém estatísticas sobre as vendas de um produto em 200 mercados diferentes, juntamente com os orçamentos de publicidade em cada um desses mercados para diferentes canais de mídia: TV, rádio e jornal.

Os testes de suposições servem para verificar se o modelo de regressão é estatisticamente válido. Mesmo que o modelo tenha um R-quadrado alto, ele pode ser inadequado se essas suposições forem violadas.
'''

df_publicidade = pd.read_csv('publicidade.csv')

print("\n******* DADOS DO DATAFRAME *******")
print(df_publicidade.head(5))

print("\n******* ESTRUTURA DO DATAFRAME *******")
print(df_publicidade.info())
...
X = df_publicidade.drop(["Vendas"],axis=1)
y = df_publicidade.Vendas
...
# ETAPA 3: Escalonamento dos dados
...
sns.pairplot(df_publicidade, x_vars=['TV','Radio','Jornal'], y_vars='Vendas',aspect=0.7)
plt.show()

'''
Observando os gráficos, podemos ver que, com a variável Vendas, nenhuma das variáveis 
independentes forma uma forma precisamente linear, mas "TV" e "Radio" ainda se saem melhor que "Journal", 
que parece não ter nenhuma forma específica.

RESUMINDO: 

- TV → relação relativamente linear

- Radio → relação relativamente linear

- Jornal → relação fraca

Isso indica que Jornal pode ter pouca capacidade explicativa.
'''
...
são homoscedásticos. Por esse motivo, podemos comprovar essa suposição.
...
'''
OBS: os termos residuais são distribuídos de forma praticamente normal para o número de pontos de teste que utilizamos. Lembre-se do teorema do limite central, 
que diz que, à medida que o tamanho da amostra aumenta, a distribuição tende a ser normal. 
Uma assimetria também é visível no gráfico. É muito difícil obter curvas e distribuições perfeitas 
...
sns.heatmap(df_publicidade.corr(), annot=True,cmap='RdYlGn',square=True)
plt.show()
...
'''
CONCLUSÃO: Portanto, os principais pressupostos da Regressão Linear foram cumpridos com sucesso. Nesse caso, podemos afirmar que, apesar de haver poucos dados, eles são bem representativos. Além disso, as variáveis independentes conseguem explicar a dependente.
'''
